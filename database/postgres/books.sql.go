...
RETURNING id, title, author, tags, price, quantity, description, created_at
`
...
select id, title, author, tags, price, quantity, description, created_at from "Books"
where id = $1 limit 1
`
...
func (q *Queries) GetBooks(ctx context.Context, arg GetBooksParams) ([]Book, error) {
	rows, err := q.db.Query(ctx, getBooks, arg.Author, arg.Limit, arg.Offset)
	if err != nil {
		return nil, err
	}
	defer rows.Close()
