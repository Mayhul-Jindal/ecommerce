...
const searchBookV2 = `-- name: SearchBookV2 :many
...
type SearchBookV2Params struct {
...
type SearchBookV2Row struct {
	...
	TagsDifference        interface{} `json:"tags_difference"`
	TitleDifference       interface{} `json:"title_difference"`
	AuthorDifference      interface{} `json:"author_difference"`
	DescriptionDifference interface{} `json:"description_difference"`
	MaxDifference         interface{} `json:"max_difference"`
}

func (q *Queries) SearchBookV2(ctx context.Context, arg SearchBookV2Params) ([]SearchBookV2Row, error) {
	rows, err := q.db.Query(ctx, searchBookV2,
	...
	defer rows.Close()
	items := []SearchBookV2Row{}
	for rows.Next() {
		var i SearchBookV2Row
		if err := rows.Scan(
			&i.ID,
			&i.Title,
			&i.Author,
